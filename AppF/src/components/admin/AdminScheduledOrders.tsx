  ...
  return (
    <div className="space-y-4 text-slate-700">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex-1 min-w-[200px]">
          <input
            ...
            className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 text-sm text-slate-800 placeholder-slate-450 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#10b981] transition-all font-medium"
          />
        </div>
        <div className="flex items-center gap-2">
          <p className="text-xs text-slate-550 font-bold whitespace-nowrap">{filtered.length} agendamento(s) pendente(s)</p>
          <button
            onClick={() => load(false)}
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-slate-550 text-sm font-semibold bg-slate-50/50 rounded-2xl border border-dashed border-slate-200 flex flex-col items-center gap-2">
            ...
            <div key={order.id} className="bg-white border border-slate-200/80 hover:border-amber-300 rounded-2xl p-4 flex flex-col justify-between shadow-sm hover:shadow transition-all space-y-3">
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div>
                    ...
                    {order.customerPhone && (
                      <p className="text-xs text-slate-550 mt-0.5 font-medium">📞 {order.customerPhone}</p>
                    )}
                ...
                <div className="space-y-2 text-[11px]">
                  {order.deliveryType === 'delivery' && order.address && (
                    <div className="text-slate-700 bg-emerald-50/30 border border-emerald-100 rounded-xl p-2.5 space-y-1">
                  ...
                  <div className="bg-slate-50 border border-slate-150 rounded-xl p-2.5 text-slate-550 flex items-start gap-2">
              ...
              <div className="flex gap-2 pt-2 border-t border-slate-100">
                <button
                  ...
                  className="flex-1 py-2 text-xs font-bold rounded-xl bg-purple-650 hover:bg-purple-750 text-white transition-all shadow-md flex items-center justify-center gap-1.5"
                >
