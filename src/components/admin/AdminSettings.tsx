  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3">
        <RefreshCw className="w-8 h-8 animate-spin text-purple-650" />
        <p className="text-sm text-slate-550 font-bold">Carregando configurações...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 text-slate-700 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2 text-purple-755">
          ...
          <form onSubmit={handleCreateCoupon} className="bg-slate-50/70 border border-slate-200/80 rounded-2xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white border border-slate-200 rounded-xl px-3 py-1.5 text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#10b981] font-bold uppercase"
                />
              </div>
              <div>
                ...
                <select
                  ...
                  className="w-full bg-white border border-slate-200 rounded-xl px-3 py-1.5 text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#10b981] font-bold"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white border border-slate-200 rounded-xl px-3 py-1.5 text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#10b981] font-medium"
                />
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-2 bg-[#10b981] hover:bg-[#059669] text-white font-bold text-xs rounded-xl shadow-md shadow-emerald-100 flex items-center justify-center gap-1.5 transition-colors disabled:opacity-50"
            >
          ...
          <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
            {coupons.length === 0 ? (
              <p className="text-xs text-slate-450 italic text-center py-6 bg-slate-50 border border-dashed rounded-xl">Sem cupons cadastrados.</p>
            ) : (
              coupons.map(coupon => (
                <div key={coupon.codigo} className="bg-white border border-slate-150 rounded-xl p-3 flex items-center justify-between shadow-sm">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-mono font-black text-xs text-purple-700 bg-purple-50 px-2 py-0.5 rounded-lg border border-purple-100">{coupon.codigo}</span>
                      <span className="text-[10px] font-extrabold uppercase px-1.5 py-0.2 rounded-full bg-slate-100 text-slate-650">
                    ...
                    <p className="text-[10px] text-slate-450 mt-0.5">{coupon.descricao}</p>
                  </div>
                  <button
                    onClick={() => handleDeleteCoupon(coupon.codigo)}
                    className="p-1.5 rounded-lg bg-slate-50 hover:bg-red-50 text-slate-455 hover:text-red-655 border border-slate-200 transition-colors"
                  >
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2 max-h-[460px] overflow-y-auto pr-1">
            {neighborhoods.length === 0 ? (
              <p className="text-xs text-slate-455 italic text-center py-6 bg-slate-50 border border-dashed rounded-xl">Sem bairros cadastrados.</p>
            ) : (
              neighborhoods.map(n => (
                <div key={n.bairro} className="bg-white border border-slate-150 rounded-xl p-3 flex items-center justify-between shadow-sm">
