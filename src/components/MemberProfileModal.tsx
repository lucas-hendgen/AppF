        ...
        <div className="flex border-b border-slate-100 bg-slate-50 px-6 gap-2 overflow-x-auto scrollbar-none">
          <button
            ...
            className={`py-3 px-3 text-xs sm:text-sm font-bold border-b-2 transition-all flex items-center gap-2 whitespace-nowrap ${
              activeTab === 'profile'
                ? 'border-[#10b981] text-[#10b981] bg-white rounded-t-xl'
        ...
        <div className="p-6 overflow-y-auto flex-1">
            ...
            <form onSubmit={handleSaveProfile} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 placeholder-slate-400 focus:ring-2 focus:ring-[#10b981] focus:outline-none"
                  />
                </div>
              ...
              <div className="flex justify-end pt-2">
                <button
                  ...
                  className="px-6 py-2.5 bg-[#10b981] hover:bg-[#059669] text-white font-bold text-sm rounded-xl shadow-lg shadow-emerald-100/30 transition-all flex items-center gap-2 disabled:opacity-50"
                >
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                {!isAddingAddress && (
                  <button
                    ...
                    className="px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 font-bold text-xs rounded-xl flex items-center gap-1.5 transition-colors shadow-sm"
                  >
              ...
              {isAddingAddress && (
                <form onSubmit={handleSaveNewAddress} className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3 animate-in fade-in">
                  <h4 className="text-xs font-bold text-emerald-700 uppercase">Cadastrar Novo Endereço em Itapema</h4>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      <label className="text-[11px] font-semibold text-slate-500">Complemento</label>
                      <input
                        type="text"
                        value={newComplement}
              ...
              <div className="space-y-2.5">
                {user.addresses && user.addresses.length > 0 ? (
                  ...
                  <div className="text-center py-8 text-slate-555 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="p-1.5 text-slate-400 hover:text-[#10b981] hover:bg-slate-50 rounded-lg"
              ...
              {loadingOrders ? (
                ...
                <div className="space-y-3">
                  {orders.map((ord) => (
                    <div
                      key={ord.id}
                      className="p-4 bg-slate-50 border border-slate-200/60 rounded-2xl shadow-sm hover:border-emerald-300 transition-all space-y-3"
                    >
                      ...
                      <div className="space-y-1">
                        {ord.items.map((it, idx) => (
                          <div key={idx} className="flex justify-between text-xs text-slate-600">
                            ...
                            <span className="font-semibold text-slate-850">R$ {(it.price * it.quantity).toFixed(2).replace('.', ',')}</span>
                          </div>
                        ))}
                      </div>
          ...
          {activeTab === 'club' && (
            <div className="space-y-5">
              <div className="p-5 bg-gradient-to-br from-amber-50 to-orange-50/50 border border-amber-200/60 rounded-3xl flex items-center justify-between">
                <div>
                  <span className="text-[11px] font-bold uppercase tracking-wider text-amber-700">
                    Seu Nível no Programa
                  </span>
                  <h3 className="text-xl font-black text-amber-800 mt-0.5 flex items-center gap-2">
                    <Sparkles className="w-5 h-5 text-amber-600" /> {user.membershipTier}
                  </h3>
                  <p className="text-xs text-slate-500 mt-1">
                    Acumule 2 pontos a cada R$ 1,00 em compras pelo aplicativo!
                  </p>
                </div>
                <div className="text-right p-3 bg-white/80 backdrop-blur rounded-2xl shadow-sm border border-amber-200">
                  <span className="text-[10px] text-amber-600 font-bold uppercase">Pontuação Atual</span>
                  <p className="text-2xl font-black text-emerald-600">{user.loyaltyPoints || 0}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="p-3.5 bg-white border border-dashed border-emerald-300 rounded-2xl">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs font-black text-[#10b981] font-mono">POPULAR10</span>
                      <span className="text-[10px] bg-emerald-50 text-emerald-700 font-bold px-2 py-0.5 rounded border border-emerald-200">10% OFF</span>
                    </div>
